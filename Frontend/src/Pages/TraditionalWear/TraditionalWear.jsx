...
import topWearList from "/public/assets/TraditionalWearItems";
import { ThemeContext } from "../../Context/ThemeContext";
import { Link } from "react-router-dom";
import CategoriesList from "../../Components/CategoriesList/EthnicWearCat";

const TopWear = () => {
  const { theme } = useContext(ThemeContext);
  const [search, setSearch] = useState("");
  const [filteredProducts, setFilteredProducts] = useState(topWearList);
  const [currentPage, setCurrentPage] = useState(1);
  const [category, setCategory] = useState("All");
  const productsPerPage = 8;

  useEffect(() => {
    let filtered = topWearList;

    if (category !== "All") {
      filtered = topWearList.filter(
