...
import FootwearList from "/public/assets/footwearItemsList";
import { ThemeContext } from "../../Context/ThemeContext";
import { Link } from "react-router-dom";
import CategoriesList from "../../Components/CategoriesList/FootWearCategories";

const Footwear = () => {
  const { theme } = useContext(ThemeContext);
  const [search, setSearch] = useState(""); 
  const [filteredProducts, setFilteredProducts] = useState(FootwearList);
  const [currentPage, setCurrentPage] = useState(1);
  const [category, setCategory] = useState("All");
  const productsPerPage = 8;

  useEffect(() => {
    let filtered = FootwearList;

    if (category !== "All") {
      filtered = FootwearList.filter(
  ...
  return (
    <div className={`p-6 lg:p-10 min-h-screen transition-all duration-300 ${theme === "dark" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-900"}`}>
